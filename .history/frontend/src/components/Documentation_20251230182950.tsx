...
import { ArrowLeft, Copy, Check, Menu, X, BookOpen, GitBranch, Code, Zap } from "lucide-react";
...
export default function Documentation({ setIsAuthenticated }: { setIsAuthenticated: (value: boolean) => void }) {
  ...
  const faq = [
    { q: "What's the difference between global and local installation?", a: "Global installation allows you to run JVCS from anywhere on your system without npx. Local installation is project-specific and requires using npx before commands." },
    { q: "How do I authenticate?", a: "Run 'jvcs begin' to login or signup. It will prompt you in the terminal for credentials." },
    { q: "Can I undo a commit?", a: "Yes, 'jvcs revert <commit-hash>' allows you to revert a commit." },
    { q: "How do I make my repository public?", a: "Use the web dashboard to change repository visibility to 'public'." },
    { q: "What if I staged the wrong files?", a: "Use 'jvcs unstage <file>' to remove files from staging before committing." },
  ];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            ...
            <div className="sticky top-24 bg-gradient-to-b from-[#1a1629]/95 to-[#0d0221]/95 backdrop-blur-xl border border-[#ff006e]/40 rounded-2xl p-6 shadow-2xl shadow-[#ff006e]/5">
              ...
              <nav className="space-y-3">
                {sections.map((section) => (
                  <button key={section.id} onClick={() => scrollToSection(section.id)} className={`w-full text-left px-4 py-3 rounded-xl transition-all duration-200 flex items-center gap-3 ${activeSection === section.id ? "bg-gradient-to-r from-[#ff006e] to-[#00d9ff] text-white shadow-lg shadow-[#ff006e]/30 scale-105" : "text-gray-400 hover:bg-[#ff006e]/15 hover:text-[#ff006e] hover:translate-x-1"}`}>
          ...
          <div className="lg:col-span-4 space-y-12">
            ...
            <section id="installation" className="scroll-mt-20 space-y-6">
              ...
              {[
                { title: "Global Installation", cmd: "npm install -g jvcs", desc: "Install JVCS globally to use anywhere in your system.", note: "Requires admin privileges on some systems." },
                { title: "Local Installation", cmd: "npm install jvcs", desc: "Install JVCS in the current project folder. Must use npx to run commands.", note: "Useful for project-specific usage." },
              ].map((inst, idx) => (
                <div key={idx} className="bg-[#0d0221] border border-[#ff006e]/40 rounded-2xl p-6 shadow-md hover:shadow-lg transition-all">
                  <h3 className="text-2xl font-bold text-[#ff006e] mb-2">{inst.title}</h3>
                  <p className="text-gray-300 mb-3">{inst.desc}</p>
                  <div className="relative mb-2">
                    <SyntaxHighlighter language="bash" style={tomorrow} customStyle={{ backgroundColor: "#1a1629", padding: "16px", borderRadius: "8px" }}>
                      {inst.cmd}
                    </SyntaxHighlighter>
                    <button onClick={() => copyToClipboard(inst.cmd, `install-${idx}`)} className="absolute top-4 right-4 p-2.5 hover:bg-[#ff006e]/20 rounded-lg transition-all">
                      {copied === `install-${idx}` ? <Check className="w-5 h-5 text-[#ffbe0b]" /> : <Copy className="w-5 h-5 text-white" />}
                    </button>
                  </div>
              ...
              {commands.map((cmd, idx) => (
                <div key={idx} className="bg-[#0d0221] border border-[#ff006e]/40 rounded-2xl p-6 shadow-md hover:shadow-lg transition-all">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => copyToClipboard(cmd.example, `cmd-${idx}`)} className="p-2 bg-[#ff006e]/20 rounded-lg hover:bg-[#ff006e]/40 transition-colors">
                      {copied === `cmd-${idx}` ? <Check className="text-[#ffbe0b]" /> : <Copy className="text-white" />}
                    </button>
                  </div>
            ...
            <section id="workflow" className="scroll-mt-20 space-y-6">
              ...
              <p className="text-gray-300 mb-4">A typical JVCS workflow:</p>
