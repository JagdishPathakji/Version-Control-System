...
export default function OwnRepo({
  ...
  const changeVisibility = async(reponame) => {
      ...
      <div className="flex flex-col">
        {nodes.map((node) => (
          <div key={node.driveId}>
            <div
              className="flex items-center py-2 px-3 border-y border-[#1f2029] cursor-pointer hover:bg-[#0e1118] hover:border-[#00ffd533] transition-all"
              ...
              {node.type === "folder" ? (
                <>
                  {openNodes[node.driveId] ? (
                    <ChevronDown className="w-4 h-4 text-[#00ffd5] mr-1" />
                  ) : (
                    <ChevronRight className="w-4 h-4 text-[#00ffd5] mr-1" />
                  )}
                  <Folder className="w-4 h-4 text-[#00ffd5] mr-2" />
                </>
              ) : (
                <File className="w-4 h-4 text-[#00ffd5] mr-2 ml-[1.2rem]" />
              )}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0d0221] via-[#1a1629] to-[#0d0221] text-gray-200 flex flex-col">
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 sm:px-6 py-10 space-y-8">
        ...
        <div className="bg-[#1a1629]/90 backdrop-blur-xl border border-[#ff006e]/30 rounded-2xl p-6 shadow-2xl hover:shadow-[0_0_50px_rgba(255,0,110,0.15)] transition-all duration-300">
          <div className="flex justify-between items-start flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-2xl font-bold bg-gradient-to-r from-[#ff006e] to-[#00d9ff] bg-clip-text text-transparent break-all">{repo.name}</h1>
            </div>

            {/* Star + Visibility */}
            <div className="flex items-center gap-3 text-gray-400 text-sm bg-[#0d0221]/60 px-3 py-2 rounded-lg border border-[#ff006e]/20">
             {isStarred !== null && (
              <button
              ...
              >
                {isStarred ? (
                  ...
                  <Star className="w-4 h-4" fill="none" stroke="#ffd700" />
                )}
                <span>{star}</span>
              </button>
            )}

              <div className="flex items-center gap-1" onClick={()=>changeVisibility(repo.name)}>
                {visibility === "public" ? (
        ...
        {/* Commit History */}
        <div className="bg-[#111217] border border-[#1f2029] rounded-2xl p-6">
          <h2 className="text-lg font-bold text-[#00ffd5] mb-6 flex items-center gap-2">
            <Clock className="w-5 h-5 text-[#00ffd5]" /> Commit History
          </h2>
          ...
          {repo.content.length === 0 ? (
            ...
            <div className="flex flex-col gap-8">
              {repo.content.map((commit) => {
                ...
                return (
                  <div key={commit._id} className="border border-[#22232c] bg-[#16181f] rounded-xl p-5">
                    <div className="flex justify-between items-center mb-3">
                      <h3 className="font-semibold text-[#00ffd5] text-sm">
  ...
  >
    ...
    <div className="sticky top-0 flex justify-between items-center px-4 sm:px-6 py-3 border-b border-[#1f2029] bg-[#0f1114]/95 backdrop-blur-sm">
      <h3 className="text-[#00ffd5] font-semibold text-sm sm:text-base truncate max-w-[80%]">
        {selectedFile.name}
      </h3>
      <button
        onClick={() => {
